...
public class Golay24
{
     ...
     */
    public static final int[] CHECKSUMS = new int[]{0x63A,0x31D,0x7B4,0x3DA,0x1ED,0x6CC,0x366,0x1B3,0x6E3,0x54B,0x49F,
        0x475,0x400,0x200,0x100,0x080,0x040,0x020,0x010,0x008,0x004,0x002,0x001};
    ...
    private static int calculateChecksum(BinaryMessage message)
    {
        int calculated = 0; //Starting value

		/* Iterate the set bits and XOR running checksum with lookup value */
        for(int i = message.nextSetBit(0); i >= 0 && i < 12; i = message.nextSetBit(i + 1))
        {
    ...
    /**
     * Performs error detection and correction using the Golay 24 code.
     *
     * @param message - 24-bit source message
     * @return - true if the message was corrected
     */
    public static boolean checkAndCorrect(BinaryMessage message)
    {
        boolean parityError = message.cardinality() % 2 != 0;

        int syndrome = getSyndrome(message);
    ...
    {
        boolean parityError = message.get(startIndex, startIndex + 24).cardinality() % 2 != 0;
    ...
    private static int getSyndrome(BinaryMessage message)
    {
        int calculated = calculateChecksum(message);

        int checksum = message.getInt(12, 22);
